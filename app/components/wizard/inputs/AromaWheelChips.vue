...
import { AROMAS } from '~/utils/wineTypeFilters'
import {
  AROMA_RING_TYPES,
  AROMA_TYPE_LABELS,
  type AromaRingType
      ...
      >
        ...
        <div class="flex flex-wrap gap-1.5">
          <span
            ...
            :data-testid="`chip-${chip.descriptor.replace(/[\s/]+/g, '-')}`"
          >
            <span class="capitalize">{{ chip.descriptor }}</span>
            <button
              type="button"
              ...
              :aria-label="`Remove ${chip.descriptor}`"
              :data-testid="`remove-${chip.descriptor.replace(/[\s/]+/g, '-')}`"
